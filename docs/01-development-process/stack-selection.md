...
| Маркетплейс на WooCommerce (стек зафиксирован) | Витрина WooCommerce + React SPA кабинет продавца | WordPress + custom mu-plugin (хуки, custom REST, Action Scheduler) | MySQL/MariaDB (+ кастомные таблицы для суб-заказов/леджера) | WP roles + Application Passwords (same-origin SPA) | VPS PHP-FPM/Nginx или Managed WP | PHP standalone+integration (wp eval-file), Vitest, smoke | Если стек свободный — предпочесть JS-маркетплейс строкой выше; если нужны сплит-платежи на чекауте с самого старта |
