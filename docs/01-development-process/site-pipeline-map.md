...
|---|------|------------------|------|-------|------|
| 1 | preflight | `new-site-preflight.ps1` | raw request | route, stack hint, audit command | `ready` или вопросы `needs-discovery` |
...
| 10 | site-backend | `site-backend` | `_architecture.md`, `_content-model.md` | код + тесты | backend/API/database checks без block |
| 11 | site-frontend | `site-frontend` | backend, `_content-model.md`, design tokens | код + browser smoke | frontend checks и browser verification |
| 12 | site-seo | `site-seo` | финальные public pages | metadata, sitemap, metrics | SEO/performance/analytics checks |
| 13 | site-review | `site-review` | реализованный проект | чеклисты, smoke, sign-off | testing + security + legal/UAT без block |
| 14 | site-deploy | `site-deploy` | passed review | staging/production URL | release/infrastructure/rollback gates |
| 15 | site-handoff | `site-handoff` | production deploy | `handoff.md` | post-deploy smoke, доступы, acceptance |
| 16 | post-release | maintenance plan | `handoff.md` | 30-90 day review plan | optional, дата/условия зафиксированы |
| 17 | capture-learnings | `capture-learnings` | evidence / approval | preferences/wiki artifacts или no artifact | learning review выполнен |
