...
Классификация двухосевая: тип продукта выбирает ровно один primary playbook; явные ограничения
платформы/рантайма становятся supporting guides и stack constraints. Смешение primary playbooks не
используется. Реестр и совместимость profiles — в `resources/site-pipeline-contract.json`.
...
| Контентный сайт, редакторы, публикации | [content-site](../13-playbooks/content-site.md) | Astro/Next.js + optional CMS | Неясны preview, media, roles, redirects |
| Существующий WordPress/editorial workflow | primary по продукту + guide [WordPress](../02-frontend/WordPress.md) | WordPress theme или headless WordPress + Astro/Next | Неясны plugin policy, roles, cache, preview |
...
| E-commerce или checkout | primary [e-commerce](../13-playbooks/ecommerce.md), optional guide [headless commerce](../13-playbooks/headless-commerce.md) | Next.js storefront + commerce/payment backend | Неясны catalog, tax, inventory, refunds, marketplace |
...
| Явный Shopify/Hydrogen | primary e-commerce + guide [Shopify Hydrogen](../13-playbooks/shopify-hydrogen.md) | Hydrogen + Shopify Storefront API + Shopify checkout | Неясны Shopify source of truth, checkout constraints |
...
2. Взять primary playbook по типу продукта и добавить соответствующий supporting guide; адаптировать
   архитектурные концепции под зафиксированный стек — концепции переносимы, реализация нет.
...
2. Chosen route: primary playbook + delivery profile + supporting guides + stack blueprint
