...
1.5. По умолчанию helper не нужен; при конкретном пробеле используй максимум один backend/API/database helper. Итоговые решения
...
## Передача дальше
`site-design` — независимая ветвь. `site-frontend` начинается после завершения design и применимого
backend и работает против реальных эндпоинтов.
