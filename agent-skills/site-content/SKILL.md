...
## Requires
- `site-architecture` завершён (контент-модель ложится на модель данных/CMS — артефакт
  `_architecture.md` проекта).
...
3.1. По умолчанию helper не нужен; при конкретном пробеле используй максимум один content helper для черновика/ревью. Факты, цены,
...
6. **Артефакт.** Сохрани в корень проекта `_content-model.md`: контент-модель (типы/поля/связи,
   что в CMS), карту страниц/навигацию, статус текстов по секциям (реальный контент или явный
   плейсхолдер), i18n-структуру и список юридических страниц. Это финальные data shapes для
   `site-backend` и вход для `site-design`/`site-frontend`/`site-seo` — и точка resume. Если файл
   уже существует — актуализируй.

## Quality gate
- Есть контент-модель и карта страниц; строки готовы к i18n (не захардкожены).
- Контент-модель сохранена в `_content-model.md` проекта.
...
## Передача дальше
После контента `site-design` и применимый `site-backend` становятся независимыми ветвями графа.
